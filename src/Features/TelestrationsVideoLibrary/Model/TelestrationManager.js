...
export default class TelestrationManager {
    constructor() {
        ...
        this.config = {
            ...
            FADE_IN_TIME: 1,
            FADE_OUT_TIME: 0.16,
            MAX_CURSOR_RADIUS: 120,
            MAX_LIGHTSHAFT_RADIUS: 120,
            MAX_ARROW_WIDTH: 50,
            MAX_Z_ANGLE: 1,
        ...
        this.FUNCTION_ENUM = {
            ...
            CHROMA_KEY_PICKER: 6,
            LIVE_MODE: 2,
            LIVE_MODE_PLACE_CURSOR: 'circle',
            NONE: 0,
            PLACE_ARROW_POINT: 'arrow',
            PLACE_LIGHT_SHAFT: 'lightshaft',
            PLACE_LINKED_CURSOR: 'linkedcursor',
            PLACE_POLYGON: 8,
            PLACE_SMOOTH_ARROW: 'freearrow',
            PLACE_STRAIGHT_ARROW: 'straightarrow',
            PLAYER_CUT_OUT: 'playercutout',
            PLACE_TEXT_BOX: 13,
        };
        ...
        this.pointerDownEventCounter = 0;
    }
    ...
    triggerUndo = function () {
        if (this.actionManager.actions.length > 0) {
    ...
    triggerRedo = function () {
        if (this.actionManager.redoArray.length > 0) {
            ...
            for (let i = 0; i < dbackground.length; i += 4) {
                ...
                if (this.chromaKey.isChromaBackground(r, g, b, a)) {
                    ...
                    if (
                        ...
                        this.FUNCTION_ENUM.CHROMA_KEY_PICKER
                    ) {
                    ...
                    let currentPlayerCutOutPosition = playerCutOutsPositions.find(
                        ({ x1, x2, y1, y2 }) =>
                            Utils.inRange(cx, x1, x2) &&
                            Utils.inRange(cy, y1, y2)
                    );
    ...
    renderWatermark = function (context) {
        ...
        if (
            userEmail // && excludedList.indexOf(userEmail) > -1
        ) {
            return;
        }
    ...
    updateCreationTelestrations = function (delta) {
        switch (this.currentFunction) {
            ...
            case this.FUNCTION_ENUM.PLAYER_CUT_OUT:
                ...
                if (this.creationObject.arrow.type === 1) {
                    this.creationObject.arrow.update(delta);
                }
                break;
        }
    };
    ...
    initializeCurrentFunction = function () {
        let previousArrowType = 0;
        if (this.creationObject) {
            //if previous tool was any arrow
            if (this.creationObject.type) {
                previousArrowType = this.creationObject.type;
                //if previous tool was player cutout
            } else if (this.creationObject.arrow) {
                previousArrowType = this.creationObject.arrow.type;
            }
        }
        const currentArrowType = this.creationObject
            ? this.creationObject.type || 0
            : 0;
        const sliderSizeInput = document.getElementById('size-slider');
        switch (this.currentFunction) {
            case this.FUNCTION_ENUM.LIVE_MODE_PLACE_CURSOR:
    ...
    captureCanvasMousePosition = function (event) {
        ...
        // this.transformMousePositionTo3d();
    };
    ...
    onmousedown = function (event) {
        ...
        switch (this.currentFunction) {
            case this.FUNCTION_ENUM.PLACE_STRAIGHT_ARROW:
                ...
                break;
            case this.FUNCTION_ENUM.PLAYER_CUT_OUT:
                if (this.pointerDownEventCounter === 0) {
                    this.onSliderChangeSize(sliderSizeInput.value);
                    this.creationObject.placePoint(
                        this.getRelativeMousePosition()
                    );
                    this.pointerDownEventCounter++;
                }
                if (
                    this.creationObject &&
    ...
    onSliderChangeSize = function (mouseStep) {
        this.changeSizeSlider(mouseStep);
    };
    ...
    ListenSliderInputs = () => {
        //access imput and adding observer to access tool side change
        const sliderSizeInput = document.getElementById('size-slider');
        ...
        const sizeInputCallback = (mutationsList, observer) => {
        ...
        const sizeInputObserver = new MutationObserver(sizeInputCallback);
        sizeInputObserver.observe(sliderSizeInput, config);
    ...
    setEvents = function () {
        ...
        this.nonRecordableCanvas.addEventListener('pointermove', (event) =>
            this.onmousemove(event)
        );
        document.addEventListener('pointerup', (event) =>
            this.onmouseup(event)
        );
        this.nonRecordableCanvas.addEventListener('pointerdown', (event) =>
