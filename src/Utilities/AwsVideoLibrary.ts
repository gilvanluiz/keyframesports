...
export interface IUploadVideo {
    ...
    saveTelestratedVideoLink: (value: string) => void;
}
...
const checkConversationStatus = async (
    ...
    if (conversation.status === 'complete') {
        if (type === 'footage') {
            ...
            if (saveTelestratedVideoLink) {
                saveTelestratedVideoLink(video_url + '.mp4');
            }
            onStartDownload();
            _downloadFile({
                name,
...
export const beamVideo = async (uploadVideoOpts: IUploadVideo) => {
    ...
    // Send to s3
    return await _beamToS3(uploadVideoOpts)
        .then(async (beamOpts) => {
...
export const s3KeyToUrlImage = (k: string) => {
    console.log('image xxxxxx', process.env.REACT_APP_S3_URI);
    const relativeUrl = k
        .split('/')
    ...
    return {
        image_url: process.env.REACT_APP_S3_URI + relativeUrl + '.jpg',
    };
};

export const s3KeyToUrl = (k: string) => {
    console.log('video xxxxxx', process.env.REACT_APP_S3_URI);
